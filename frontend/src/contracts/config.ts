...
export const CONTRACT_ADDRESSES = {
  calibration: {
    FilecoinCIDStore: process.env.NEXT_PUBLIC_CALIBRATION_CID_STORE_ADDRESS || '',
  },
  filecoin: {
...
export const addFilecoinNetwork = async (network: 'calibration' | 'filecoin') => {
        ...
        params: [
          {
            ...
            blockExplorerUrls: [FILECOIN_NETWORKS[network].blockExplorers.default.url],
          },
        ],
      });
    } catch (error) {
...
export const switchToFilecoinNetwork = async (network: 'calibration' | 'filecoin') => {
  if (typeof window !== 'undefined' && window.ethereum) {
    try {
      await window.ethereum.request({
        method: 'wallet_switchEthereumChain',
        params: [{ chainId: `0x${FILECOIN_NETWORKS[network].chainId.toString(16)}` }],
      });
